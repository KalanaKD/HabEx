  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center gap-2">
        ...
        <h1 className="flex-1 text-xl font-bold text-ink">Categories</h1>
      ...
      {editing && (
        <form onSubmit={save} className="space-y-3 rounded-xl bg-surface p-4 shadow">
          <div>
            ...
            <div className="grid grid-cols-3 gap-2">
              {GROUPS.map((g) => (
                <button key={g} type="button" onClick={() => setGroup(g)}
                  className={`rounded-lg border px-2 py-2 text-sm capitalize ${group === g ? 'border-accent bg-accent-soft text-accent' : 'border-edge-strong text-ink-muted'}`}>
                  {g}
                </button>
              ))}
            </div>
          ...
          <div className="flex gap-2">
            ...
            <button type="button" onClick={() => setEditing(null)} className="rounded-lg border border-edge-strong px-4 text-ink-muted">Cancel</button>
            {editing !== 'new' && (
      ...
      <ul className="divide-y divide-edge rounded-xl bg-surface shadow">
        {categories.map((c) => (
          <li key={c.id} className="flex items-center gap-3 p-3">
            ...
            <button onClick={() => startEdit(c)} aria-label={`Edit ${c.name}`} className="p-1 text-ink-faint"><LuPencil /></button>
          </li>
        ))}
      </ul>
    </div>
